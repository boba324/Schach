...
namespace Schach
{
    ...
    {
        //Konstruktor
        public Turm(bool weiss, int zeile, int spalte, bool firstMove, ref Figur[,] feld)
            : base(weiss, zeile, spalte, firstMove, ref feld)
        {
            this.ImageFile = "RookSmall";
        }
        ...
        public override bool kannBetreten(int zeile, int spalte)
        {
            for (int i = 8; i > 0; i--)
            {
                if ((this.Zeile - i) == zeile && (this.Spalte) == spalte) return istNichtsImWeg(zeile, spalte);
            }
            for (int i = 8; i > 0; i--)
            {
                if ((this.Zeile) == zeile && (this.Spalte - i) == spalte) return istNichtsImWeg(zeile, spalte);
            }
            for (int i = 1; i < 8; i++)
            {
                if ((this.Zeile) == zeile && (this.Spalte + i) == spalte) return istNichtsImWeg(zeile, spalte);
